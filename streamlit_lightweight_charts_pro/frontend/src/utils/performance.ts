...
export const createOptimizedStyles = memoize(
  (width: number | null, height: number | null, shouldAutoSize: boolean, chartOptions: any) => {
    return {
      container: {
        ...
        borderRadius: '0px',
        padding: '0px',
        width: shouldAutoSize || width === null ? '100%' : (typeof width === 'number' ? `${width}px` : width || '100%'),
        height: shouldAutoSize ? '100%' : (typeof height === 'number' ? `${height}px` : height || '100%'),
