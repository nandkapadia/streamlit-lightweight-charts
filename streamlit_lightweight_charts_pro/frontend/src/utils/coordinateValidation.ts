...
export function validateChartCoordinates(coordinates: ChartCoordinates): ValidationResult {
  ...
  if (!coordinates.container) {
    ...
    if (coordinates.container.width < DIMENSIONS.chart.minWidth) {
      warnings.push(`Container width (${coordinates.container.width}) is below recommended minimum (${DIMENSIONS.chart.minWidth})`);
    }
    if (coordinates.container.height < DIMENSIONS.chart.minHeight) {
      warnings.push(`Container height (${coordinates.container.height}) is below recommended minimum (${DIMENSIONS.chart.minHeight})`);
    }
  }
  ...
  if (!coordinates.timeScale) {
    ...
    if (coordinates.timeScale.height < DIMENSIONS.timeAxis.minHeight) {
      warnings.push(`Time scale height (${coordinates.timeScale.height}) is below minimum (${DIMENSIONS.timeAxis.minHeight})`);
    }
    if (coordinates.timeScale.height > DIMENSIONS.timeAxis.maxHeight) {
      warnings.push(`Time scale height (${coordinates.timeScale.height}) exceeds maximum (${DIMENSIONS.timeAxis.maxHeight})`);
    }
  }
...
export function logValidationResult(result: ValidationResult, context: string = ''): void {
  ...
  if (result.isValid && result.warnings.length === 0) {
    console.log(`${prefix}✅ Coordinates validated successfully`);
  }
}
