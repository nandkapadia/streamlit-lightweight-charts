...
class TestRealWorldTradingDashboardSimple:
    ...
    def test_tradingview_equivalent_dashboard(self):
        ...
        # Create main price chart with volume using new Chart method
        price_volume_chart = Chart.from_price_volume_dataframe(price_df, price_type="candlestick")
        ...
        dashboard = [price_volume_chart, sma_chart, rsi_chart]
    ...
    def test_portfolio_analysis_dashboard(self):
        ...
        allocation_chart = Chart(series=[HistogramSeries(data=allocation_data, color="#4CAF50")])
        ...
        # Verify performance chart has correct number of series
    ...
    def _create_allocation_data(self, portfolio_data):
        ...
        for i, (asset_name, asset_info) in enumerate(portfolio_data.items()):
            allocation_data.append(
                SingleValueData(
                    time=base_date + pd.Timedelta(days=i),  # Use a valid date for time
    ...
    def test_trading_dashboard_performance(self):
        ...
        price_chart = Chart.from_price_volume_dataframe(large_price_df, price_type="candlestick")
        sma_chart = Chart(series=[LineSeries(self._calculate_sma(large_price_data, 20))])
        rsi_chart = Chart(series=[LineSeries(self._calculate_rsi(large_price_data, 14))])

        # Test performance
        import time

        start_time = time.time()
        price_config = price_chart.to_frontend_config()
    ...
    def test_trading_dashboard_integration_workflow(self):
        ...
        dashboard_charts = self._create_comprehensive_dashboard(
            market_data, volume_data, sma_20, rsi, signals
        )

        # 5. Test each chart's configuration individually
        price_config = dashboard_charts[0].to_frontend_config()
        sma_config = dashboard_charts[1].to_frontend_config()
        rsi_config = dashboard_charts[2].to_frontend_config()
        ...
        # Verify price chart has series
        price_chart = price_config["charts"][0]
        assert "chart" in price_chart
        assert "series" in price_chart
        assert len(price_chart["series"]) >= 1  # At least candlestick series
    ...
    def _create_comprehensive_dashboard(self, price_data, volume_data, sma_20, rsi, signals):
        ...
        if signals:
            ...
            dashboard = [price_chart, sma_chart, rsi_chart]
