...
class TestHistogramSeriesCreateVolumeSeries:
    ...
    def test_series_properties(self):
        ...
        df = pd.DataFrame({"time": ["2024-01-01"], "volume": [1000], "open": [100], "close": [105]})
        ...
        assert isinstance(volume_series, HistogramSeries)
        assert volume_series.pane_id == 1
        assert volume_series.price_scale_id == "left"
        assert len(volume_series.data) == 1
    ...
    def test_empty_dataframe_handling(self):
        ...
        df = pd.DataFrame(columns=["time", "volume", "open", "close"])
