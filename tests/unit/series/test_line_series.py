...
def test_from_dataframe(df, column_mapping, line_options):
    series = LineSeries.from_dataframe(df, column_mapping, line_options=line_options)
...
def test_missing_required_column_in_mapping(df, line_options):
    ...
    with pytest.raises(ValueError):
        LineSeries.from_dataframe(df, bad_mapping, line_options=line_options)
...
def test_missing_required_column_in_dataframe(line_options):
    ...
    with pytest.raises(ValueError):
        LineSeries.from_dataframe(bad_df, mapping, line_options=line_options)
...
def test_extra_columns_in_dataframe(line_options):
    ...
    mapping = {"time": "datetime", "value": "close", "color": "color"}
    series = LineSeries.from_dataframe(df, mapping, line_options=line_options)
