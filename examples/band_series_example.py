...
def generate_sample_data(periods=100):
    ...
    for i, (date, close) in enumerate(zip(dates, prices)):
        ...
        if i == 0:
            ...
            open_price = ohlc_data[-1][ColumnNames.CLOSE]

        ohlc_data.append(
            {
                ColumnNames.DATETIME: date,
                ColumnNames.OPEN: open_price,
                ColumnNames.HIGH: high,
                ColumnNames.LOW: low,
                ColumnNames.CLOSE: close,
            }
        )
...
def calculate_bollinger_bands(df, period=20, std_dev=2):
    ...
    close_prices = df[ColumnNames.CLOSE]
    ...
    return pd.DataFrame(
        {
            ColumnNames.DATETIME: df[ColumnNames.DATETIME],
            "upper": upper_band,
            "middle": sma,
            "lower": lower_band,
        }
...
def calculate_keltner_channels(df, period=20, multiplier=2):
    ...
    typical_price = (df[ColumnNames.HIGH] + df[ColumnNames.LOW] + df[ColumnNames.CLOSE]) / 3

    # Calculate EMA of typical price
    ema = typical_price.ewm(span=period).mean()

    # Calculate Average True Range (ATR)
    high_low = df[ColumnNames.HIGH] - df[ColumnNames.LOW]
    high_close = np.abs(df[ColumnNames.HIGH] - df[ColumnNames.CLOSE].shift())
    low_close = np.abs(df[ColumnNames.LOW] - df[ColumnNames.CLOSE].shift())
...
def main():
    ...
    st.set_page_config(page_title="Band Series Examples", page_icon="📊", layout="wide")
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📈 Bollinger Bands", "📊 Keltner Channels", "🎨 Custom Bands", "🔗 Overlay Example"]
    )
    ...
    with tab1:
        ...
        **Bollinger Bands** are a technical analysis tool consisting of:
        ...
        for i, row in ohlc_df.iterrows():
            ...
            close_price = row[ColumnNames.CLOSE]
            ...
            if abs(close_price - upper_band) < 0.5:
                bollinger_series.add_marker(
                    time=str(row[ColumnNames.DATETIME]),
        ...
        with col2:
            st.metric(
                "Upper Band Range",
                f"{bollinger_df['upper'].min():.2f} - {bollinger_df['upper'].max():.2f}",
            )
        with col3:
            st.metric(
                "Lower Band Range",
                f"{bollinger_df['lower'].min():.2f} - {bollinger_df['lower'].max():.2f}",
            )
    ...
    with tab2:
        ...
        **Keltner Channels** are volatility-based bands consisting of:
        ...
        with col2:
            st.metric(
                "Upper Channel Range",
                f"{keltner_df['upper'].min():.2f} - {keltner_df['upper'].max():.2f}",
            )
        with col3:
            st.metric(
                "Lower Channel Range",
                f"{keltner_df['lower'].min():.2f} - {keltner_df['lower'].max():.2f}",
            )
    ...
    with tab3:
        ...
        **Custom Bands** demonstrate the flexibility of BandSeries for creating
        ...
        custom_df = pd.DataFrame(
            {
                ColumnNames.DATETIME: ohlc_df[ColumnNames.DATETIME],
                "upper": ohlc_df[ColumnNames.CLOSE] * 1.1,  # 10% above close
                "middle": ohlc_df[ColumnNames.CLOSE],  # Close price
                "lower": ohlc_df[ColumnNames.CLOSE] * 0.9,  # 10% below close
            }
        )
        ...
        with col2:
            st.metric(
                "Upper Band Range",
                f"{custom_df['upper'].min():.2f} - {custom_df['upper'].max():.2f}",
            )
        with col3:
            st.metric(
                "Lower Band Range",
                f"{custom_df['lower'].min():.2f} - {custom_df['lower'].max():.2f}",
            )
    ...
    with st.sidebar:
        ...
        std_dev = st.slider(
            "Standard Deviation", 1.0, 3.0, 2.0, step=0.1, help="Multiplier for standard deviation"
        )

        st.subheader("Styling Options")
        line_width = st.slider("Line Width", 1, 5, 2, help="Width of band lines")
        fill_opacity = st.slider(
            "Fill Opacity", 0.0, 1.0, 0.1, step=0.05, help="Opacity of fill areas"
        )

        line_type = st.selectbox(
            "Line Type", ["Simple", "Curved"], help="Type of line interpolation"
        )
        ...
        st.metric(
            "Date Range",
            f"{ohlc_df['datetime'].min().strftime('%Y-%m-%d')} to {ohlc_df['datetime'].max().strftime('%Y-%m-%d')}",
        )
