...
def calculate_supertrend(df):
    """Calculate simplified Supertrend values for demonstration."""
    trend_fill_data = []
    ...
    for i in range(len(df)):
        if i < 20:  # First 20 periods
            ...
            if price_change > 2:  # Uptrend
                trend_direction = 1
                upper_trend = None
                lower_trend = df.iloc[i]['close'] - 5 - (i % 10) * 0.1
            elif price_change < -2:  # Downtrend
                trend_direction = -1
                upper_trend = df.iloc[i]['close'] + 5 + (i % 10) * 0.1
                lower_trend = None
            else:  # Neutral
                trend_direction = 0
                upper_trend = None
...
def main():
    ...
    trend_fill_data = calculate_supertrend(df)
                ...
                "data": [
                    {
                        ...
                        "upper_trend": item.upper_trend,
                        "lower_trend": item.lower_trend,
