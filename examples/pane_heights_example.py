...
from streamlit_lightweight_charts_pro import (
    ...
    LineSeries, LineData, HistogramSeries, HistogramData,
    CandlestickSeries, CandlestickData, AreaSeries, AreaData
)
...
sma_20_data = [
    LineData(time=int(dt.timestamp()), value=sma)
    for dt, sma in zip(dates, sma_20)
]

sma_50_data = [
    LineData(time=int(dt.timestamp()), value=sma)
    for dt, sma in zip(dates, sma_50)
]
...
rsi_data = [
    LineData(time=int(dt.timestamp()), value=rsi)
    for dt, rsi in zip(dates, rsi_values)
]
...
with tab3:
    ...
    dashboard_chart = Chart(
        ...
        series=[
            ...
            LineSeries(
                data=[LineData(time=int(dt.timestamp()), value=((prices[i] - prices[i-1]) / prices[i-1]) * 100) 
                      for i, dt in enumerate(dates) if i > 0],
                pane_id=3
            )
        ]
    )
