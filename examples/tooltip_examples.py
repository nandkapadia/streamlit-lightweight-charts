...
from streamlit_lightweight_charts_pro.data import (
    SingleValueData, CandlestickData, AreaData, TradeData, BarMarker,
    create_ohlc_tooltip, create_custom_tooltip, create_trade_tooltip,
    create_single_value_tooltip, TooltipConfig, TooltipField, TooltipManager,
    TooltipType, TooltipPosition, TooltipStyle
)
from streamlit_lightweight_charts_pro.type_definitions.enums import TradeType, MarkerPosition, MarkerShape
...
def example_basic_ohlc_tooltip():
    ...
    chart = Chart(
        series=CandlestickSeries(data=ohlc_data[:50])
    )
...
def example_custom_template_tooltip():
    ...
    chart = Chart(
        series=LineSeries(data=line_data[:50])
    )
...
def example_multi_series_tooltip():
    ...
    line_tooltip.fields = [
        TooltipField("MA20", "value", precision=2, prefix="$", color="#ff6b6b")
    ]
    ...
    volume_tooltip.fields = [
        TooltipField("Volume", "value", formatter=lambda x: f"{x:,.0f}")
    ]
...
def example_tooltip_manager():
    ...
    ma_tooltip.fields = [
        TooltipField("MA20", "value", precision=2, prefix="$")
    ]
...
def main():
    ...
    st.write("This page demonstrates the comprehensive tooltip functionality with various use cases.")
    
    # Create tabs for different examples
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "Basic OHLC", "Custom Template", "Multi-Series", 
        "Trade Tooltips", "Styled Tooltips", "Tooltip Manager", "Marker Tooltips"
    ])
