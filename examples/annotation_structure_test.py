...
from streamlit_lightweight_charts_pro import Chart, CandlestickSeries, LineSeries
...
from streamlit_lightweight_charts_pro.data.annotation import (
    Annotation, create_text_annotation, create_arrow_annotation, AnnotationManager
)
...
def main():
    ...
    st.write("This example tests the annotation structure handling to ensure that annotations from the Python AnnotationManager are properly processed by the frontend.")
    ...
    st.write("This chart uses the AnnotationManager from the Python side, which creates a 'layers' structure.")
    
    chart_with_manager = Chart(
        ChartOptions(
            width=800,
            height=400
        )
    ...
    # Add candlestick series
    candlestick_series2 = CandlestickSeries(
        data=ohlcv_data,
        name="Price",
        color="#FF9800"
    )
    ...
    # Add candlestick series
    candlestick_series3 = CandlestickSeries(
        data=ohlcv_data,
        name="Price",
        color="#9C27B0"
    )
    ...
    st.success("✅ All charts should render without errors, properly handling both annotation structures!")
